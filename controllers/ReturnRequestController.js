  ...
  try {
    ...
    if (order.Delivery !== 'delivered') {
      return res.status(400).json({
        success: false,
        message: 'Return request is allowed after delivery only.',
      });
    }
    ...
    const populatedRequest = await populateReturnRequest(
      ReturnRequest.findById(request._id)
    );
...
export const getMyReturnRequests = async (req, res) => {
  try {
    const requests = await populateReturnRequest(
      ReturnRequest.find({ user: req.user._id }).sort({ createdAt: -1 })
    );
...
export const getReturnRequests = async (req, res) => {
  try {
    const requests = await populateReturnRequest(
      ReturnRequest.find().sort({ createdAt: -1 })
    );
